...
Removes a PHP extension from a specific PHP version.

### Description

This command removes a whitelisted PHP extension from the specified PHP version. The extension and its configuration files are purged from the system.
...
```json
{
  "success": false,
  "error": {
    "code": "EXTENSION_REMOVE_FAILED",
    "message": "Failed to remove PHP extension redis: E: Unable to locate package php8.2-redis"
  }
}
```
...
|------------|-------------|
| `VALIDATION_ERROR` | Invalid PHP version or extension not in whitelist |
| `EXTENSION_REMOVE_FAILED` | Package removal failed |

### Notes

- Removal timeout: 300 seconds (5 minutes)
- Uses `apt-get remove -y --purge` to remove the package and configuration
- Extension names should not include the `php{version}-` prefix
- Only extensions from the allowed whitelist can be removed (same list as install)
